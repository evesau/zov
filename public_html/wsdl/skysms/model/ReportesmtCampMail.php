...
sql;
    ...
    public static function getMTListData($data, $start, $length){
      mail("esau.espinoza@airmovil.com", "ejecuta mail ReportesmtCampMail", print_r($data,1));

      $mysqli = DatabaseDao::getInstance(1,1);
...
sql;
      ...
      $datos = $mysqli->queryAll($query, $params);

      //mail("esau.espinoza@airmovil.com","datos", print_r($datos, 1) . $query . " \nstart: $start - length: $length" );
...
sql;
  ...
  public static function getDatareporte(){
    $mysqli = DatabaseDao::getInstance();
    $query =<<<sql
SELECT * FROM reporte_correo WHERE enviado = 0 ORDER BY ctime ASC LIMIT 1
sql;
    return $mysqli->queryOne($query);
  }

  public static function updateReporteenviado($process, $id){
    $mysqli = DatabaseDao::getInstance();
    $query =<<<sql
UPDATE reporte_correo SET enviado=$process WHERE reporte_correo_id=$id
sql;
