...
//@mail("tecnico@airmovil.com","TESTING SmsSkyWsSecure.php","SOAP: $soap\n SERVER:".print_r($_SERVER,1));
...
XML;
    echo $xml;
    mail("tecnico@airmovil.com","empty soap secure","soap vacio");
    exit();
}
*/
...
$server = new SoapServer("https://smsmkt.amovil.mx/wsdl/skysms/SmsSkyWsSecure.wsdl");
...
function sendSms($params) // Funcionando OK
{
	...
	//mail("$tecnico", 'SOAP sendSms ws-multi', $input. "\nparams: \n" . print_r($params,1) ."--". print_r($_SERVER,1). "--VALIDA: ---$valida---usuarioGlobal:$usuarioGlobal ------");
	...
	$passwordBD = (array) ApiSmsSkyWs::getUser($usuarioGlobal);
...
html;
	...
	$login = ApiSmsSkyWs::getByIdLoginWs($userData); // se necesitan estos datos para sacar campaing , customer_id y user_id para control de mt's
	...
	if ($valida) {
		...
		if ($customer_id == 1) {
			...
			if ($usuarioGlobal == 'skyservice') {
				$campaign_id = 404; // campaign_id solo para sky id 233 short 59850 256 short 76551, short 349 short 24766, 350 short 12345, short 404 12345
				$ccsc_id = 24;
			} elseif ($usuarioGlobal == 'skyccc') {
				$campaign_id = 401; // campaign_id solo para sky id 233 short 59850 256 short 76551, short 401 short 24766,
				$ccsc_id = 23;
			} elseif ($usuarioGlobal == 'skyvetv') {
				...
				if ($totalMesRestaUser>=1) {
					...
					if ($totalDiaRestaCustomer>=1) {
						...
						if ($totalMesRestaCustomer>=1) {
							...
							//mail("$tecnico","tolerancia",print_r($tolerancia,1));
...
html;
												...
												# Encolamos sms
													...
													//mail("$tecnico","insertSmsCampaign msisdn new","$insertarSms: msisdn:$msisdn params:($campaign_id, $msisdnId, -1, 10, 4, $mensaje)");
													if ($insertarSms > 0) {
														$idMessage = $insertarSms;
														$operador = $carrier;
														$headerResponse='';
...
html;
	...
	//$fechaSoap = date('Y-m-d H:i:s');
	//mail("tecnico@airmovil.com","SOAP SKY FIN EN SENDSMS id:$idSoap fecha:$fechaSoap",print_r($_SERVER,1));
	exit;
}
...
html;
	...
	$login = ApiSmsSkyWs::getByIdLoginWs($userData); // obtener que la sms_campaig_id le corresponda al usucario
...
html;
	...
	//$fechaSoap = date('Y-m-d H:i:s');
	//mail("tecnico@airmovil.com","SOAP SKY FIN EN statusSms id:$idSoap fecha:$fechaSoap",print_r($_SERVER,1));
	exit;
...
html;
	...
	$login = ApiSmsSkyWs::getByIdLoginWs($userData); //ṕara poder hacer el cambio de pass de acuerdo al user_id
...
html;
	...
	//$fechaSoap = date('Y-m-d H:i:s');
	//mail("tecnico@airmovil.com","SOAP SKY FIN en changePasswordSms id:$idSoap fecha:$fechaSoap",print_r($_SERVER,1));
	exit;
}
	...
	<SOAP-ENV:Envelope
		...
		<SOAP-ENV:Body>
			<ns2:BalanceSmsResponse>
				<ns2:netBalance>100</ns2:netBalance>
				<ns2:usedBalance>10</ns2:usedBalance>
				<ns2:balance>90</ns2:balance>
				<ns2:dateQuery>2018-08-23 00:00:00</ns2:dateQuery>
...
html;
	...
	//mail("$tecnico", 'SOAP balanceSms ws-multi', $input. "\nparams: \n" . print_r($params,1) ."--\n". print_r($_SERVER,1)."---usuarioGlobal:$usuarioGlobal-----valida: $valida---");
...
html;
	...
	$login = ApiSmsSkyWs::getByIdLoginWs($userData); // para sacar mt's restantes por customer
...
function exchangeSms($params)
{
	...
	$login = ApiSmsSkyWs::getByIdLoginWs($userData); // obtner user y customer
	if ($valida) {
...
html;
	...
	//$fechaSoap = date('Y-m-d H:i:s');
	//mail("tecnico@airmovil.com","SOAP SKY FIN en exchangeSms id:$idSoap fecha:$fechaSoap",print_r($_SERVER,1));
	exit;
}
...
html;
	...
	//$fechaSoap = date('Y-m-d H:i:s');
	//mail("tecnico@airmovil.com","SOAP SKY FIN en incomingSms id:$idSoap fecha:$fechaSoap",print_r($_SERVER,1));
	exit;
}
...
function Security($data){
	...
	$tecnico ='tecnico@airmovil.com';
	//mail("$tecnico","security login",print_r($data,1).print_r($_SERVER,1).print_r($input,1)."-----valida: $valida----");
	...
	//mail("$tecnico", "expired", print_r($expired,1).count($expired));
...
html;
	        mail("$tecnico","Usuario Bloqueado SOAP","Usuario:$Username - password:$PasswordDigest".json_encode($timeLoked));
			...
			//mail("$tecnico",'expire preuba esau',print_r($expired,1). print_r(count($expired),1));
...
html;
				...
				<ns2:headerResponse>
					<ns1:codeError>-12</ns1:codeError>
		            <ns1:messageError>Password Expired</ns1:messageError>
		        </ns2:headerResponse>
html;
...
html;
	...
	mail("$tecnico", "Response error login", "Usuario:$Username - password:$PasswordDigest\n".print_r($response,1).print_r($data,1));
