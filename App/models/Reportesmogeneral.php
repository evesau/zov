...
class Reportesmogeneral implements Crud{
...
sql;
	...
	public static function getAllCampaignMT($id){
		...
		SELECT c.campaign_id, c.name AS name_campaign, c.delivery_date, sc.short_code
		FROM campaign c
		LEFT JOIN campaign_api_web USING (campaign_id)
		INNER JOIN campaign_customer cc using(campaign_id)
		INNER JOIN customer cu on (cc.customer_id = cu.customer_id)
		INNER JOIN carrier_connection_short_code_campaign ccscc using(campaign_id)
		INNER JOIN carrier_connection_short_code ccsc on (ccscc.carrier_connection_short_code_id = ccsc.carrier_connection_short_code_id)
		INNER JOIN short_code sc on (sc.short_code_id = ccsc.short_code_id)
		WHERE cu.customer_id = 1 AND MONTH(c.delivery_date) = MONTH(CURDATE()) AND campaign_api_web_id IS NULL
		GROUP BY c.campaign_id
sql;
...
############################################ INICIO BUSQUEDA MT CAMPANIA & PARAMETROS ######################################################
	...
	public static function getTotalSearchReporteMT($id, $destination){ # Destination total
		...
		*
		FROM msisdn m
		LEFT JOIN sms_campaign sc USING ( msisdn_id )
		LEFT JOIN sms_campaign_estatus sce USING ( sms_campaign_estatus_id )
		LEFT JOIN sms s ON ( m.msisdn = s.destination )
		LEFT JOIN carrier_connection_short_code ccsc ON ( sc.carrier_connection_short_code_id = ccsc.carrier_connection_short_code_id )
		LEFT JOIN short_code sco USING ( short_code_id )
		LEFT JOIN carrier c ON (ccsc.carrier_id =c.carrier_id )
		WHERE (sc.campaign_id =$id OR s.campaign_id =$id)
		AND m.msisdn LIKE "%$destination%"
		GROUP BY sms_campaign_id
sql;
	...
	public static function getTotalSearchReporteMTCop($id, $destination, $start, $length){ #destination data
		...
		c.name AS "CARRIER",
		IF(s.status IS NULL, sce.estatus, s.status) AS "ESTATUSSMS",
		IF(s.entry_time IS NULL,sc.delivery_date, s.entry_time) AS "FECHA",
		IF(s.content IS NULL, sc.content, s.content) AS "CONTENT",
		IF(s.destination IS NULL, m.msisdn,s.destination) AS "DESTINATION",
		IF(s.source IS NULL,sco.short_code, s.source) AS "SHORTCODE"
		FROM msisdn m
		LEFT JOIN sms_campaign sc USING ( msisdn_id )
		LEFT JOIN sms_campaign_estatus sce USING ( sms_campaign_estatus_id )
		LEFT JOIN sms s ON ( m.msisdn = s.destination )
...
DISTINCT (sms_campaign_id) AS unico,
		...
		msisdn_log AS DESTINATION,
			...
			IF (s.status IS NULL, sce.estatus, status) AS ESTATUSSMS
...
GROUP BY sms_campaign_id
*/
	public static function reportMT($id, $result){
		$mysqli = Database::getInstance();
		$query = <<<sql
SELECT *
FROM sms
...
INNER JOIN carrier c ON ( ccsc.carrier_id = c.carrier_id )
INNER JOIN short_code scd ON ( scd.short_code_id = ccsc.short_code_id )
WHERE sms.campaign_id = :result  AND cc.customer_id = :id
GROUP BY sms_id 
sql;
...
####################################### REPORTE EXCEL CON PARAMETROS ######################################################
	public static function reportMTE($id_campania){
		...
		LEFT JOIN short_code sco USING ( short_code_id )
		LEFT JOIN carrier c ON (ccsc.carrier_id =c.carrier_id )
		WHERE (sc.campaign_id =$id_campania OR s.campaign_id =$id_campania)
		GROUP BY sms_campaign_id
sql;
...
############################################### FIN BUSQUEDA MT CAMPANIA & PARAMETROS & FECHAS ###############################################
    ...
    public static function registroUsuario($registro)
    {
        $mysqli = Database::getInstance();
        $query=<<<sql
			INSERT INTO user_registro(ctime, user_id, nickname, name_customer, script, ip, modulo,accion) 
			VALUES (now(),:id_user,:nickname,:name_customer,:script,:ip,:modulo,:accion);
sql;
        $params = array(
...
sql;
	...
	public static function getSmsServicio($consulta){
		$mysqli = Database::getInstance();
		$query =<<<sql
		SELECT 
			m.entry_time,
		...
		ON m.carrier_connection_short_code_id = ccsc.carrier_connection_short_code_id
		JOIN carrier c 
		ON ccsc.carrier_id = c.carrier_id
		WHERE cs.customer_id = $consulta->customer_id $consulta->where
		$consulta->limit
sql;
		return $mysqli->queryAll($query);
	}
	...
	public static function reportMOGeneral($consulta){
		...
		$consulta->limit
sql;
		//mail('jorge.manon@airmovil.com','Query Reportes','Query reportMOGeneral: <br>'.$query);
		return $mysqli->queryAll($query);
	}

	public static function getAllCampaignMO($id){
		//$and = ($short_codes != '')? " AND sc.short_code IN($short_codes)": '';
		$mysqli = Database::getInstance();
		$query = <<<sql
		SELECT c.campaign_id, c.name, sc.short_code
		FROM campaign c
...
sql;
	...
	public static function getShortCodeByCustomer($customer_id){
		$mysqli = Database::getInstance();
		$query =<<<sql
		SELECT DISTINCT(sc.short_code) AS SHORT_CODE
		FROM custome_carrier_connection_short_code cccsc
		JOIN carrier_connection_short_code ccsc
