...
class ReportesmtCamp implements Crud{
    ...
    public static function getMTAllCount($data){
      //mail("tecnico@airmovil.com", "prueba loteId", print_r($data,1));
      ...
      if ($consulta == 1) {
        ...
        if (!empty($data->loteid)) {
          $params[':loteid'] .= ($data->loteid==0) ? ' ' : $data->loteid;
          $andloteid = "AND sc.loteId=:loteid ";
        }
...
sql;
    ...
    public static function getMTListData($data, $start, $length){
      //mail("tecnico@airmovil.com", "getMTListData", print_r($data,1));
...
sql;
      ...
      //mail("tecnico@airmovil.com","Query filtro limit", print_r($params, 1) . $query . " \nstart: $start - length: $length" );
...
sql;
  ...
  public static function insertReporte($data){
    $mysqli = Database::getInstance(1,1);
    $query =<<<sql
INSERT INTO reporte_correo ( ctime, correo, customer_id, campaign_id, fecha_inicio, fecha_fin, carrier, destination, source, estatus, loteid, enviado) VALUES (NOW(), :correo, :customer_id, :campaign_id, :fecha_inicio, :fecha_fin, :carrier, :destination, :source, :estatus, :loteid, 0)
sql;
  ...
  public static function getAllCorreos($data){
    $mysqli = Database::getInstance();
    $query =<<<sql
SELECT * FROM reporte_correo WHERE customer_id = :customer_id
sql;
    
    return $mysqli->queryAll($query, array(':customer_id' => $data->customer_id));
  }

  public static function getCorreosListData($data, $start, $length){
    $mysqli = Database::getInstance();
    $query =<<<sql
SELECT 
ctime as fecha, 
correo, 
campaign_id as campania, 
fecha_inicio, 
fecha_fin, 
carrier, 
destination, 
...
(
    CASE
    WHEN enviado=0 THEN 'En espera'
    WHEN enviado=-1 THEN 'Procesando'
    WHEN enviado=1 THEN 'Enviado'
    ELSE 'Error'
    END
) as enviado,
(
    ...
    WHEN enviado=1 
      THEN CONCAT(
            CONCAT(
              CONCAT('<div>
                        <a href="/Reportemail/reenviar/',reporte_correo_id),'" class="btn btn-info" role="button" style="width: 100%; height: 100%;"> Reenviar </a>
                        <a href="/Reportemail/borrar/',reporte_correo_id),'" class="btn btn-danger" role="button" style="width: 100%; height: 100%;"> Borrar </a>
                      </div>')
    END
) as accion
FROM reporte_correo
...
sql;
  ...
  public static function borrarCorreo($id){
    $mysqli = Database::getInstance();
    $query =<<<sql
DELETE FROM reporte_correo WHERE reporte_correo_id = :id
sql;
    return $mysqli->delete($query,array(':id' => $id));
  }

  public static function reenviaCorreo($id){
    $mysqli = Database::getInstance();
    $query =<<<sql
UPDATE reporte_correo SET enviado=0 WHERE reporte_correo_id = :id
sql;
