...
class Reportesapi implements Crud{
...
############################################ INICIO BUSQUEDA MT CAMPANIA & PARAMETROS ######################################################
	...
	public static function getTotalSearchReporteMT($id, $destination){ # Destination total
		$mysqli = Database::getInstance();
		$query = <<<sql
SELECT s.entry_time AS FECHA, c.name AS CARRIER, destination AS DESTINATION, source AS SHORTCODE, s.content AS CONTENT, 
IF(s.status IS NULL ,sce.estatus,s.status) AS ESTATUSSMS
FROM sms AS s
LEFT JOIN sms_campaign sc ON ( s.campaign_id = sc.campaign_id AND s.destination = sc.msisdn_log ) 
LEFT JOIN carrier_connection_short_code AS ccsc ON ( s.carrier_connection_short_code_id = ccsc.carrier_connection_short_code_id ) 
LEFT JOIN carrier AS c USING ( carrier_id )
LEFT JOIN sms_campaign_estatus sce USING ( sms_campaign_estatus_id ) 
...
IF(s.status IS NULL ,sce.estatus,s.status) AS ESTATUSSMS
		FROM sms AS s
		LEFT JOIN sms_campaign sc ON ( s.campaign_id = sc.campaign_id
		AND s.destination = sc.msisdn_log )
		LEFT JOIN carrier_connection_short_code AS ccsc ON ( s.carrier_connection_short_code_id = ccsc.carrier_connection_short_code_id )
...
IF(s.status IS NULL ,sce.estatus,s.status) AS ESTATUSSMS
		...
		LEFT JOIN sms_campaign_estatus sce USING ( sms_campaign_estatus_id ) 
		WHERE s.campaign_id = $id
		AND s.destination LIKE '$destination'
		AND (s.status LIKE '$estatus')
		GROUP BY s.sms_id
sql;
...
sql;
	...
	public static function getDataSMSApi($id){
		$mysqli = Database::getInstance();
		$id = (int)$id;
		$query = <<<sql
SELECT *
...
IF(c.name IS NULL, (SELECT carrier.name from carrier INNER JOIN msisdn USING (carrier_id) WHERE  msisdn.msisdn='s.destination' ORDER BY msisdn_id),c.name) AS "CARRIER",
...
WHERE (sc.campaign_id =$result OR s.campaign_id =$result)
GROUP BY s.sms_id
LIMIT $start, $length
sql;
...
LEFT JOIN carrier AS c
USING ( carrier_id )
WHERE s.campaign_id = $id_campania
GROUP BY s.sms_id
sql;
