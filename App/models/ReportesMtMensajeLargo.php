...
class ReportesMtMensajeLargo implements Crud{
...
SELECT
	sc.ctime AS FECHA,
  	IF(sms.carrier ='' OR sms.carrier is null,  IF(ca.name is null,'sin carrier',ca.name), sms.carrier) AS CARRIER,
  	...
  	IF(sms.content is null OR sms.content ='', sc.content, sms.content) AS CONTENT,
  	IF(sms.status_dlr !='',
    	sms.status_dlr,
...
$datos->where AND sc.secuencia != '' AND c.name LIKE 'message_large_%'
GROUP BY sc.sms_campaign_id
ORDER BY c.campaign_id,sc.secuencia ASC
sql;
		mail('jorge.manon@airmovil.com','Query Reportes','Query getDataBuscarTest https://smsmkt.amovil.mx/ReportesMtMensajeLargo/mt: <br> '.$query);
...
WHERE cu.customer_id = $customer_id AND c.name LIKE 'message_large_%'
sql;
		mail('jorge.manon@airmovil.com', 'Query Reportes', 'Query getMaxCampaign: '.$query);
...
AND c.name LIKE 'message_large_%'
GROUP BY c.campaign_id;
sql;
		mail('jorge.manon@airmovil.com', 'Query Reportes', 'Query getAllCampaign: '.$query);
...
GROUP BY c.campaign_id;
sql;
		mail('jorge.manon@airmovil.com', 'Query Reportes', 'Query getAllCampaignMtYear: '.$query);
		return $mysqli->queryAll($query, array(':id'=>$id));
	}
...
WHERE cu.customer_id = $id AND c.name LIKE 'message_large_%'
...
INNER JOIN short_code sc USING ( short_code_id )
WHERE cu.customer_id = $id AND MONTH( c.delivery_date ) = MONTH( CURDATE( ) )
AND c.delivery_date BETWEEN '$fecha_ini' AND '$fecha_fin' AND c.name LIKE 'message_large_%'
GROUP BY c.campaign_id
sql;
		return $mysqli->queryAll($query);
	}
...
INNER JOIN carrier_connection_short_code ccsc USING ( carrier_connection_short_code_id )
INNER JOIN short_code sc USING ( short_code_id )
WHERE cu.customer_id = $id AND MONTH( c.delivery_date ) = MONTH( CURDATE( ) ) AND c.name LIKE 'message_large_%'
AND cs.name LIKE '%$status%'
...
INNER JOIN carrier_connection_short_code ccsc USING ( carrier_connection_short_code_id )
INNER JOIN short_code sc USING ( short_code_id )
WHERE cu.customer_id = $id AND MONTH( c.delivery_date ) = MONTH( CURDATE( ) ) AND c.name LIKE '%$campania%' AND sc.short_code LIKE '%$shortcode%' AND cs.name LIKE '%$status%' AND m.name LIKE '%$direction%' AND c.delivery_date BETWEEN '$fecha_ini' AND '$fecha_fin' AND c.name LIKE 'message_large_%'
GROUP BY c.campaign_id
sql;
		return $mysqli->queryAll($query);
	}
...
WHERE cc.customer_id =:id AND sc.campaign_id = :result AND sc.msisdn_id LIKE "%$destination%" AND c.name LIKE 'message_large_%'
